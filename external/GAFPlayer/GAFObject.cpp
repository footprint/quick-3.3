...
#include "CCLuaEngine.h" //by footprint+++
...
void GAFObject::processAnimations(float dt)
{
    ...
    while (m_timeDelta >= frameTime)
    {
        ...
        if (m_nScriptHandler)
        {
            ...
            stack->pushInt(EvntType::FramePlayed);
            stack->pushInt(m_currentFrame);
...
void GAFObject::registerScriptHandler(cocos2d::LUA_FUNCTION handler)
{
    unregisterScriptHandler();
    m_nScriptHandler = handler;
}

void GAFObject::unregisterScriptHandler()
{
