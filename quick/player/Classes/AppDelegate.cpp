...
static int getDataFromFile(lua_State *L)
{
    ...
    cocos2d::Data content = cocos2d::FileUtils::getInstance()->getDataFromFile(file);
    lua_pushlstring(L, (const char*)content.getBytes(), content.getSize());
    return 1; /* number of results */
}

static void extend_module_register(lua_State *L)
{
    lua_getglobal(L, "_G");
    if (lua_istable(L, -1))//stack:...,_G,
    {
        lua_register(L, "getDataFromFile", getDataFromFile);
    }
    lua_pop(L, 1);
}
...
bool AppDelegate::applicationDidFinishLaunching()
{
    ...
    extend_module_register(L);

    LuaStack* stack = engine->getLuaStack();
    stack->setXXTEAKeyAndSign("YMUCJLHC3pTOV8ZH", strlen("YMUCJLHC3pTOV8ZH"), "diguotech", strlen("diguotech"));
